...
Route::middleware('auth:api')->group(function(){
    ...
    if ($user) {
        // Cargamos la relación para que no sea null
        $user->load('operador'); 
        
        // Agregamos el operador_id directamente al objeto user
        // Así el front recibe el objeto tal cual lo esperaba + el nuevo campo
...
Route::group(['middleware' => ['role:operador|supervisor']], function () {
    ...
    Route::get('citas/next-ticket', [App\Http\Controllers\CitasController::class, 'getNextTicket']); 
    Route::post('citas/cupos-config', [App\Http\Controllers\CitasController::class, 'storeCuposConfig']);
